...
//------------------------------------------------------------------------------
// File size in bytes.
const uint32_t FILE_SIZE = 1048567UL * FILE_SIZE_MB;
...
void loop() {
  ...
  for (uint16_t nTest = 0; nTest < READ_COUNT; nTest++) {
    //a random number from 0 to 1000
    randNumber = random(1000);


    uint32_t m = micros();
    
    uint64_t nextPos = randNumber * BUF_SIZE;
  ...
  cout << BUF_SIZE* READ_COUNT / t << ',' << maxLatency << ',' << minLatency;
