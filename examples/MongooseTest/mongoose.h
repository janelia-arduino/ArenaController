...
#define strdup(s) ((char *)mg_strdup (mg_str (s)).buf)
  ...
  static inline void *
  mg_calloc (size_t cnt, size_t size)
  {
...
#define mkdir(a, b) mg_mkdir (a, b)
  static inline int
  mg_mkdir (const char *path, mode_t mode)
  {
    (void)path, (void)mode;
    return -1;
  }
...
#define getpeername(a, b, c) mg_getpeername ((a), (b), (c))
  ...
  static inline int
  mg_getsockname (MG_SOCKET_TYPE fd, void *buf, socklen_t *len)
  {
...
#define mg_str(s) mg_str_s (s)
  ...
  bool mg_match (struct mg_str str, struct mg_str pattern,
                 struct mg_str *caps);
  bool mg_span (struct mg_str s, struct mg_str *a, struct mg_str *b,
                char delim);
  ...
  size_t mg_queue_book (struct mg_queue *, char **buf,
                        size_t);                     // Reserve space
...
#define MG_TIMER_RUN_NOW 2 // Call immediately when timer is set
  ...
  void mg_timer_init (struct mg_timer **head, struct mg_timer *timer,
                      uint64_t milliseconds, unsigned flags,
...
platform (1, 4, or 8 bytes). If MG_UECC_WORD_SIZE is not explicitly defined
then it will be automatically set based on your platform. */
...
/* Optimization level; trade speed for code size.
   ...
   platforms where more than one curve is enabled. */
...
/* MG_UECC_VLI_NATIVE_LITTLE_ENDIAN - If enabled (defined as nonzero), this
will switch to native little-endian format for *all* arrays passed in and out
of the public API. This includes public and private keys, shared secrets,
signatures and message hashes. Using this switch reduces the amount of call
stack memory used by uECC, since less intermediate translations are required.
Note that this will *only* work on native little-endian processors and it will
treat the uint8_t arrays passed into the public API as word arrays, therefore
requiring the provided byte arrays to be word aligned on architectures that do
not support unaligned accesses. IMPORTANT: Keys and signatures generated with
MG_UECC_VLI_NATIVE_LITTLE_ENDIAN=1 are incompatible with keys and signatures
generated with MG_UECC_VLI_NATIVE_LITTLE_ENDIAN=0; all parties must use the
same endianness. */
...
#define MG_UECC_SUPPORT_COMPRESSED_POINT 1
  ...
  extern "C"
  {
    ...
    return 1 if 'dest' was filled with random data, or 0 if the random data
    could not be generated. The filled-in values should be either truly random,
    or from a cryptographically-secure PRNG.
    ...
    A correct RNG function is set by default when building for Windows, Linux,
    or OS X. If you are building on another POSIX-compliant system that
    supports /dev/random or /dev/urandom, you can define MG_UECC_POSIX to use
    the predefined RNG. For embedded platforms there is no predefined RNG
    function; you must provide your own.
    */
    ...
    function should return 1 if the random data was generated, or 0 if the
    random data could not be generated.

    On platforms where there is no predefined RNG function (eg embedded
    platforms), this must be called before mg_uecc_make_key() or mg_uecc_sign()
    are used.
    ...
    * the curve size (in bytes) long. For example, if the curve is secp256r1,
    ...
    private key. Must be as long as the curve order; this is typically the same
    as the curve size, except for secp160r1. For example, if the curve is
    secp256r1, private_key must be 32 bytes long.

                      For secp160r1, private_key must be 21 bytes long! Note
    that the first byte will almost always be 0 (there is about a 1 in 2^80
    chance of it being non-zero).
    ...
    */
    ...
    key. If the public key is not from a trusted source and has not been
    previously verified, you should verify it first using
    mg_uecc_valid_public_key(). Note: It is recommended that you hash the
    result of mg_uecc_shared_secret() before using it for symmetric encryption
    or HMAC.
    ...
    same size as the curve size; for example, if the curve is secp256r1, secret
    ...
    */
    ...
    at least (curve size + 1) bytes long; for example, if the curve is
    secp256r1, compressed must be 33 bytes long.
    */
    ...
    */
    ...
    Note that you are not required to check for a valid public key before using
    any other uECC functions. However, you may wish to avoid spending CPU time
    computing a shared secret or verifying a signature using an invalid public
    key.
    ...
    */
    int mg_uecc_compute_public_key (const uint8_t *private_key,
                                    uint8_t *public_key, MG_UECC_Curve curve);
    ...
    least 2 * curve size long. For example, if the curve is secp256r1,
    ...
    */
    ...
    &finish_SHA256, 64, 32, tmp}}; mg_uecc_sign_deterministic(key,
    message_hash, &ctx.uECC, signature);
    }
    */
    typedef struct MG_UECC_HashContext
    {
      ...
      void (*finish_hash) (const struct MG_UECC_HashContext *context,
                           uint8_t *hash_result);
      unsigned block_size;  /* Hash function block size in bytes, eg 64 for
                               SHA-256. */
      unsigned result_size; /* Hash function result size in bytes, eg 32 for
                               SHA-256. */
    ...
    mg_uecc_set_rng() before calling this function; however, if the RNG is
    defined it will improve resistance to side-channel attacks.

    Usage: Compute a hash of the data you wish to sign (SHA-2 is recommended)
    and pass it to this function along with your private key and a hash
    context. Note that the message_hash does not need to be computed with the
    same hash function used by hash_context.
...
#define MG_UECC_ENABLE_VLI_API 0
  ...
  extern "C"
  {
    ...
    void mg_uecc_vli_mmod_fast (mg_uecc_word_t *result,
                                mg_uecc_word_t *product, MG_UECC_Curve curve);
    ...
    /* Multiplies a point by a scalar. Points are represented by the X
       coordinate followed by the Y coordinate in the same array, both
       coordinates are curve->num_words long. Note that scalar must be
       curve->num_n_words long (NOT curve->num_words). */
...
#define RFC_8439_NONCE_SIZE (12)
    ...
    /*
        ...
        input:
            ...
            - plain_text: data to be encrypted, pointer + size should not
       overlap with cipher_text pointer

        output:
            - cipher_text: encrypted plain_text with a tag appended. Make sure
       to allocate at least plain_text_size + RFC_8439_TAG_SIZE
    ...
    */
    ...
    /*
        Decrypt/unseal cipher text given the right key, nonce, and additional
       data.
       ...
       for the same key. A counter or a pseudo-random value are fine.
        ...
        output:
            - plain_text: data to be encrypted, pointer + size should not
       overlap with cipher_text pointer, leave at least enough room for
        ...
        returns:
            - size of bytes written to plain_text, -1 signals either:
                ...
                - overlapping pointers are passed for plain_text and
       cipher_text
    */
...
#define TLS_RSA_H
  ...
  struct mg_connection
  {
    ...
    unsigned is_tls_throttled
        : 1;                  // Last TLS write: MG_SOCK_PENDING() was true
  ...
  void mg_http_write_chunk (struct mg_connection *c, const char *buf,
                            size_t len);
  void mg_http_delete_chunk (struct mg_connection *c,
  ...
  void mg_http_serve_file (struct mg_connection *, struct mg_http_message *hm,
                           const char *path,
                           const struct mg_http_serve_opts *);
  void mg_http_reply (struct mg_connection *, int status_code,
                      const char *headers, const char *body_fmt, ...);
  struct mg_str *mg_http_get_header (struct mg_http_message *,
                                     const char *name);
  struct mg_str mg_http_var (struct mg_str buf, struct mg_str name);
  int mg_http_get_var (const struct mg_str *, const char *name, char *,
                       size_t);
  int mg_url_decode (const char *s, size_t n, char *to, size_t to_len,
                     int form);
...
#define WEBSOCKET_OP_PONG 10
  ...
  void mg_ws_upgrade (struct mg_connection *, struct mg_http_message *,
                      const char *fmt, ...);
  size_t mg_ws_send (struct mg_connection *, const void *buf, size_t len,
                     int op);
...
#define MQTT_PROP_SHARED_SUBSCRIPTION_AVAILABLE 0x2A
  ...
  struct mg_connection *mg_mqtt_listen (struct mg_mgr *mgr, const char *url,
                                        mg_event_handler_t fn, void *fn_data);
  void mg_mqtt_login (struct mg_connection *c,
                      const struct mg_mqtt_opts *opts);
  uint16_t mg_mqtt_pub (struct mg_connection *c,
                        const struct mg_mqtt_opts *opts);
...
#define MG_JSON_MAX_DEPTH 30
  ...
  struct mg_rpc_req
  {
    ...
    struct mg_str
        frame; // Request, e.g. {"id":1,"method":"add","params":[1,2]}
  };
  ...
  void mg_rpc_del (struct mg_rpc **head,
...
#define MG_IRAM
  ...
  bool mg_ota_write (const void *buf,
                     size_t len); // Write chunk, aligned to 1k
  ...
  bool mg_ota_flash_write (const void *buf, size_t len,
                           struct mg_flash *flash);
...
#define MG_WIFI_BAND_5G 1
  ...
  struct mg_tcpip_driver
  {
    ...
    size_t (*tx) (const void *, size_t,
                  struct mg_tcpip_if *); // Transmit frame
    size_t (*rx) (void *buf, size_t len,
                  struct mg_tcpip_if *);       // Receive frame
  ...
  enum
  {
    MG_TCPIP_EV_ST_CHG,    // state change                   uint8_t *
                           // (&ifp->state)
...
#define MG_TCPIP_STATE_READY 4 // Interface has fully come up, ready to work
  ...
  void mg_tcpip_arp_request (struct mg_tcpip_if *ifp, uint32_t ip,
                             uint8_t *mac);
  ...
  struct mg_tcpip_spi
  {
    ...
    uint8_t (*txn) (void *,
                    uint8_t); // SPI transaction: write 1 byte, read reply
  };
...
#include "pico/unique_id.h"  // keep this include
  ...
  struct mg_tcpip_driver_pico_w_data
  {
    ...
    uint8_t apchannel;
    bool apmode; // start in AP mode; 'false' starts connection to 'ssid' if
                 // not NULL
  };
...
#define MG_TCPIP_DRIVER_INIT(mgr)                                             \
  ...
  struct mg_tcpip_driver_rw612_data
  {
    ...
    // MDC clock frequency must not exceed 2.5 MHz and is calculated as
    // follows: MDC_freq = P_clock / ((mdc_cr + 1) * 2), where P_clock is the
...
#define MG_TCPIP_DRIVER_INIT(mgr)                                             \
  ...
  struct mg_tcpip_driver_tm4c_data
  {
    ...
    //    100-150 MHz   SYSCLK/62      1  <-- value for EK-TM4C129* on max
    //    speed 20-35 MHz     SYSCLK/16      2 35-60 MHz     SYSCLK/26      3
    //    0x4-0xF Reserved
