...
static QEvt const panel_set_transferred_evt = {PANEL_SET_TRANSFERRED_SIG, 0U,
                                               0U};
...
void BSP::ledOff() { digitalWriteFast(LED_BUILTIN, LOW); }

void BSP::ledOn() { digitalWriteFast(LED_BUILTIN, HIGH); }
...
void BSP::feedWatchdog() { bsp_global::wdt.feed(); }
...
void BSP::writeSerialBinaryResponse(
    uint8_t response[AC::constants::byte_count_per_response_max],
    uint8_t response_byte_count) {
  bsp_global::serial_communication_interface_stream.write(response,
                                                          response_byte_count);
}

void BSP::readSerialStringCommand(char* const command_str, char first_char) {
...
void BSP::pollEthernet() { mongoose_poll(); }
...
void BSP::writeEthernetBinaryResponse(
...
void BSP::armRefreshTimer(uint32_t frequency_hz, void (*isr)()) {
...
void BSP::fillFrameBufferWithAllOn(uint8_t* const buffer, bool grayscale) {
          ...
          for (uint8_t quarter_panel_row_index = 0;
            ...
            for (uint8_t pixel_row_index = 0;
              ...
              for (uint8_t byte_index = 0;
                ...
                // uint8_t panel_col_index = region_panel_col_index +
                // region_index * constants::panel_count_per_region_col_max;
              }
            }
          }
        }
...
uint16_t BSP::decodePatternFrameBuffer(
  ...
  for (int8_t frame_panel_row_index =
    ...
    for (uint8_t quarter_panel_col_index = 0;
         quarter_panel_col_index < constants::quarter_panel_count_per_panel_col;
         ++quarter_panel_col_index)
    ...
    // quarter_panel_col_index>=0; --quarter_panel_col_index)
    {
      ...
      for (int8_t quarter_panel_row_index =
        ...
        for (uint8_t frame_panel_col_index = 0;
             frame_panel_col_index < constants::panel_count_per_frame_col;
             ++frame_panel_col_index)
        // for (int8_t frame_panel_col_index =
        // (constants::panel_count_per_frame_col - 1); frame_panel_col_index>=0;
        // --frame_panel_col_index)
        {
          ...
          bsp_global::QuarterPanel& quarter_panel =
              bsp_global::decoded_frame.regions[region_index]
                  .panels[region_panel_row_index][region_panel_col_index]
        ...
        {
          for (uint8_t byte_index = 0;
               byte_index < byte_count_per_quarter_panel_row; ++byte_index)
          // for (int8_t byte_index = (byte_count_per_quarter_panel_row - 1);
          // byte_index>=0; --byte_index)
          {
            for (uint8_t frame_panel_col_index = 0;
                 frame_panel_col_index < constants::panel_count_per_frame_col;
                 ++frame_panel_col_index)
...
bool BSP::sortPatternFilenames() {
  ...
  while (
    ...
    char* pattern_filename_buffer =
        bsp_global::pattern_filename_array[bsp_global::pattern_file_count++];
...
void BSP::closePatternFile() { bsp_global::pattern_file.close(); }
...
void BSP::readPatternFrameFromFileIntoBuffer(
...
//............................................................................
void QV::onIdle() {  // called with interrupts DISABLED
#ifdef NDEBUG
  ...
  if (len > 0U) {  // any space available in the output buffer?
    ...
    if (buf) {
      bsp_global::qs_serial_stream.write(buf,
...
//............................................................................
QP::QSTimeCtr QP::QS::onGetTime() { return millis(); }
//............................................................................
void QP::QS::onFlush() {
