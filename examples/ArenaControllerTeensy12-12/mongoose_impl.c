...
struct apihandler_data s_apihandler_leds
    = { { "leds", "data", false, 0, 0, 0UL },
        s_leds_attributes,
        sizeof (struct leds),
        (void (*) (void *))glue_get_leds,
        (void (*) (void *))glue_set_leds };

static struct apihandler *s_apihandlers[]
    = { (struct apihandler *)&s_apihandler_leds };

static struct apihandler *
get_api_handler (struct mg_str name)
{
...
static struct apihandler *
find_handler (struct mg_http_message *hm)
{
...
void
mg_json_get_str2 (struct mg_str json, const char *path, char *buf, size_t len)
{
...
void
mongoose_set_http_handlers (const char *name, ...)
{
  ...
  if (h == NULL)
    ...
    {
      ((struct apihandler_custom *)h)->reply = va_arg (
          ap, void (*) (struct mg_connection *, struct mg_http_message *));
    }
  else
    {
...
static struct user *
authenticate (struct mg_http_message *hm)
{
  ...
  if (user[0] != '\0' && pass[0] != '\0')
    {
      ...
      if (level > 0)
        { // Proceed only if the firmware authenticated us
...
static void
handle_login (struct mg_connection *c, struct user *u)
{
  char cookie[256];
...
static void
handle_logout (struct mg_connection *c)
{
  char cookie[256];
  mg_snprintf (cookie, sizeof (cookie),
...
static void
close_uploaded_file (struct upload_state *us)
{
  us->marker = 0;
...
static void
upload_handler (struct mg_connection *c, int ev, void *ev_data)
{
  ...
  if (sizeof (*us) > sizeof (c->data))
    {
      mg_error (
          c,
          "FAILURE: sizeof(c->data) == %lu, need %lu. Set -DMG_DATA_SIZE=XXX",
  ...
  if (us->marker == 'U' && ev == MG_EV_READ && us->expected > 0
      && c->recv.len > 0)
    {
      ...
      bool ok
          = aligned > 0 ? us->fn_write (us->fp, c->recv.buf, aligned) : true;
      us->received += aligned;
  ...
  if (us->marker == 'U' && ev == MG_EV_CLOSE)
    close_uploaded_file (us);
  (void)ev_data;
}

static void
prep_upload (struct mg_connection *c, struct mg_http_message *hm,
             void *(*fn_open) (char *, size_t), bool (*fn_close) (void *),
...
static void
handle_uploads (struct mg_connection *c, int ev, void *ev_data)
{
  struct upload_state *us = (struct upload_state *)c->data;
...
static void
handle_action (struct mg_connection *c, struct mg_http_message *hm,
               bool (*check_fn) (void), void (*start_fn) (struct mg_str))
{
  if (hm->body.len > 0)
    {
      start_fn (hm->body);
...
size_t
print_struct (void (*out) (char, void *), void *ptr, va_list *ap)
{
  ...
  for (i = 0; a[i].name != NULL; i++)
    {
      ...
      if (strcmp (a[i].type, "int") == 0)
        ...
        {
          len += mg_xprintf (out, ptr, "%s",
                             *(bool *)attrptr ? "true" : "false");
        }
...
static void
handle_object (struct mg_connection *c, struct mg_http_message *hm,
  ...
  if (hm->body.len > 0 && h->data_size > 0)
    {
      ...
      if (memcmp (data, tmp, h->data_size) != 0)
        s_device_change_version++;
      if (h->setter != NULL)
        h->setter (tmp); // Can be NULL if readonly
      free (tmp);
...
static size_t
print_array (void (*out) (char, void *), void *ptr, va_list *ap)
{
...
static void
handle_array (struct mg_connection *c, struct mg_http_message *hm,
              struct apihandler_array *h)
{
  ...
  mg_http_reply (c, 200, JSON_HEADERS, "{%m:%llu, %m:%llu, %m:[%M]}\n",
                 MG_ESC ("size"), size, MG_ESC ("start"), start,
                 MG_ESC ("data"), print_array, h, &size, &start);
}

size_t
print_timeseries (void (*out) (char, void *), void *ptr, va_list *ap)
{
  ...
  for (i = 0; i < count; i++)
    {
      ...
      len += mg_xprintf (out, ptr, "%s[%lu,%g]", comma, timestamps[i],
                         values[i]);
    }
  return len;
}
...
static void
handle_api_call (struct mg_connection *c, struct mg_http_message *hm,
                 struct apihandler *h)
{
...
static void
http_ev_handler (struct mg_connection *c, int ev, void *ev_data)
{
      ...
      if (mg_match (hm->uri, mg_str ("/api/login"), NULL))
      ...
      else if (mg_match (hm->uri, mg_str ("/api/heartbeat"), NULL))
        {
          mg_http_reply (c, 200, JSON_HEADERS, "{%m:%lu}\n",
                         MG_ESC ("version"), s_device_change_version);
        }
      else if (h != NULL)
        {
...
void
mongoose_add_ws_handler (unsigned ms, void (*fn) (struct mg_connection *))
{
  ...
  for (c = g_mgr.conns; c != NULL; c = c->next)
    {
      ...
      if (c->is_websocket == 0)
        continue; // Not a websocket connection? Skip
      if (c->send.len > 2048)
        continue; // Too much data already? Skip

      for (i = 0; i < s_ws_handlers_count; i++)
        {
          if (c->pfn_data == NULL
              || mg_timer_expired (&timers[i], s_ws_handlers[i].timeout_ms,
                                   now))
            {
...
static void
sntp_ev_handler (struct mg_connection *c, int ev, void *ev_data)
{
...
static void
mqtt_event_handler (struct mg_connection *c, int ev, void *ev_data)
{
  if (ev == MG_EV_CONNECT)
    {
...
static void
handle_modbus_pdu (struct mg_connection *c, uint8_t *buf, size_t len)
{
          ...
          if ((size_t)(num * 2 + 10) < sizeof (response))
            {
              for (i = 0; i < num; i++)
                {
                  success = glue_modbus_write_reg ((uint16_t)(start + i),
                                                   mg_htons (data[i]));
                  if (success == false)
                    break;
                }
...
static void
modbus_ev_handler (struct mg_connection *c, int ev, void *ev_data)
{
...
static void
dns_fn (struct mg_connection *c, int ev, void *ev_data)
{
  if (ev == MG_EV_READ)
    {
      ...
      if (n > 0)
        {
          ...
          h->txnid
              = ((struct mg_dns_header *)c->recv.buf)->txnid; // Copy tnxid
          ...
          memcpy (buf + sizeof (*h), c->recv.buf + sizeof (*h),
                  n); // Copy question
          memcpy (buf + sizeof (*h) + n, answer,
                  sizeof (answer)); // And answer
#if MG_ENABLE_TCPIP
