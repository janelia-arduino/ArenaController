...
struct apihandler_data s_apihandler_leds = {{"leds", "data", false, 0, 0, 0UL},
                                            s_leds_attributes,
                                            sizeof(struct leds),
...
void mongoose_set_http_handlers(const char* name, ...) {
  ...
  if (h == NULL) {
    ...
    ((struct apihandler_ota*)h)->opener = va_arg(ap, void* (*)(char*, size_t));
...
void glue_update_state(void) { s_device_change_version++; }
