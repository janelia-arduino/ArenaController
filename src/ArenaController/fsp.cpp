...
static QEvt const initialize_analog_input_evt = {INITIALIZE_ANALOG_INPUT_SIG, 0U, 0U};
static QEvt const analog_input_initialized_evt = {ANALOG_INPUT_INITIALIZED_SIG, 0U, 0U};
}
}
//----------------------------------------------------------------------------
// Local functions
...
{
  static QF_MPOOL_EL(UnsignedValueEvt) smlPoolSto[constants::set_parameter_event_pool_count];
...
{
  UnsignedValueEvt *set_analog_output_ev = Q_NEW(UnsignedValueEvt, SET_ANALOG_OUTPUT_SIG);
...
void FSP::SerialCommandInterface_storePlayPatternParameters(QActive * const ao, QEvt const * e)
{
  SerialCommandInterface * const sci = static_cast<SerialCommandInterface * const>(ao);
...
void FSP::SerialCommandInterface_storeAnalogClosedLoopParameters(QActive * const ao, QEvt const * e)
{
  SerialCommandInterface * const sci = static_cast<SerialCommandInterface * const>(ao);
  AnalogClosedLoopEvt const * aclev = static_cast<AnalogClosedLoopEvt const *>(e);

  sci->runtime_duration_ms_ = aclev->runtime_duration * constants::milliseconds_per_runtime_duration_unit;
}
...
void FSP::EthernetCommandInterface_storePlayPatternParameters(QActive * const ao, QEvt const * e)
{
  EthernetCommandInterface * const eci = static_cast<EthernetCommandInterface * const>(ao);
  PlayPatternEvt const * ppev = static_cast<PlayPatternEvt const *>(e);
...
void FSP::EthernetCommandInterface_storeAnalogClosedLoopParameters(QActive * const ao, QEvt const * e)
{
  EthernetCommandInterface * const eci = static_cast<EthernetCommandInterface * const>(ao);
  AnalogClosedLoopEvt const * aclev = static_cast<AnalogClosedLoopEvt const *>(e);

  eci->runtime_duration_ms_ = aclev->runtime_duration * constants::milliseconds_per_runtime_duration_unit;
}
...
void FSP::Pattern_displayFrame(QActive * const ao, QEvt const * e)
{
  ...
  Pattern * const pattern = static_cast<Pattern * const>(ao);
  UnsignedValueEvt *uvev = Q_NEW(UnsignedValueEvt, SET_ANALOG_OUTPUT_SIG);
  uvev->value = frameIndexToAnalogOutputValue(pattern->frame_index_, pattern->frame_count_per_pattern_);
  AO_Arena->POST(uvev, ao);
  AO_Display->POST(&constants::display_frame_evt, ao);
}
...
void FSP::Card_checkPattern(QHsm * const hsm, QEvt const * e)
{
  ...
  UnsignedValueEvt *set_grayscale_ev = Q_NEW(UnsignedValueEvt, SET_GRAYSCALE_SIG);
  set_grayscale_ev->value = grayscale;
  AO_Frame->POST(set_grayscale_ev, AO_Pattern);

  UnsignedValueEvt *set_frame_count_ev = Q_NEW(UnsignedValueEvt, SET_FRAME_COUNT_PER_PATTERN_SIG);
  set_frame_count_ev->value = pattern_header.frame_count_x;
  AO_Pattern->POST(set_frame_count_ev, AO_Pattern);

  UnsignedValueEvt *set_byte_count_ev = Q_NEW(UnsignedValueEvt, SET_BYTE_COUNT_PER_FRAME_SIG);
  ...
  UnsignedValueEvt *pattern_valid_ev = Q_NEW(UnsignedValueEvt, PATTERN_VALID_SIG);
    ...
    uint8_t response[constants::byte_count_per_response_max])
{
  ...
  switch (command_byte)
  {
    ...
    case SET_FRAME_POSITION_CMD:
    {
      ...
      UnsignedValueEvt *uvev = Q_NEW(UnsignedValueEvt, UPDATE_PATTERN_FRAME_SIG);
